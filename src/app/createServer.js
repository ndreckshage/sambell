...
const render = (req, res) => {
  ...
  // { enableThunk, enableLoop, enableClientRender },

  if (enableReactRouter) {
    match({ routes: entry, location: req.url }, (routingErr, routingRedirectLocation, props) => {
      if (routingErr) {
        ...
        loadStateOnServer(serverOpts, (loadDataErr, loadDataRedirectLocation, initialData, scriptString) => {
          if (loadDataErr) {
            ...
            const appHtml = getAppHtml(props, store, initialData, reducers, enableReduxLoop);
...
const createServer = () => {
  const { port, render: enableRender } = server;

  let finalRender = (req, res) => res.status(200).send(getHtml());
  if (enableRender) finalRender = render;
