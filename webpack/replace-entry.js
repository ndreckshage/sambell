...
const getAdjustedChunkPath = chunkPath =>
  typeof chunkPath === 'object' ? chunkPath[0] : chunkPath;

const filesOnly = clientEntriesByChunk =>
  Object.keys(clientEntriesByChunk).reduce((acc, key) => {
...
module.exports = (
  ...
  const clientManifestEntry = getAdjustedChunkPath(
    clientEntriesByChunk.manifest,
  );
  ...
  fs.readFile(serverPath, 'utf8', (serverFileErr, serverEntryContents) => {
    ...
    fs.readFile(clientManifestPath, 'utf8', (manifestErr, manifestContents) => {
      ...
      serverEntryContents = serverEntryContents
        .replace(
          /'{{SAMBELL_CLIENT_WEBPACK_MANIFEST}}'/g,
          `\`${manifestContents.replace(manifestSourceMap, '')}\``,
        )
        .replace(
          /{{SAMBELL_WEBPACK_PUBLIC_PATH}}/g,
          clientConfig.output.publicPath,
        )
        .replace(/{{SAMBELL_CLIENT_VENDOR_ENTRY}}/g, clientVendorEntry)
        .replace(/{{SAMBELL_CLIENT_ENTRY}}/g, clientEntry)
        .replace(
          /{{SAMBELL_CLIENT_CHUNKS}}/g,
          JSON.stringify(filesOnly(clientEntriesByChunk)),
        );
